...
var workfrontApiConstants = require('workfront-api-constants');
...
var NodeFormData__namespace = /*#__PURE__*/_interopNamespaceDefault(NodeFormData);
...
var Api = /** @class */ (function () {
    ...
    Api.prototype.execute = function (objCode, objID, action, actionArgs) {
        ...
        if (actionArgs) {
            params = tslib.__assign(tslib.__assign({}, params), actionArgs);
        }
    ...
    Api.prototype.get = function (objCode, objIDs, fields) {
        ...
        if (objIDs.length === 1) {
            if (objIDs[0].indexOf(workfrontApiConstants.INTERNAL_PREFIX) === 0) {
                params = { id: objIDs[0] };
            }
            else {
                endPoint += '/' + objIDs[0];
    ...
    Api.prototype.report = function (objCode, query, useHttpPost) {
        ...
        if (useHttpPost) {
            reportQuery = tslib.__assign(tslib.__assign({}, query), { method: Api.Methods.GET });
    ...
    Api.prototype.request = function (path, params, fields, method) {
        ...
        var clonedParams = tslib.__assign({}, params);
    ...
    Api.prototype.search = function (objCode, query, fields, useHttpPost) {
        ...
        if (useHttpPost) {
            searchQuery = tslib.__assign(tslib.__assign({}, query), { method: Api.Methods.GET });
            method = Api.Methods.POST;
        }
        else {
            searchQuery = query;
    ...
    Api.prototype.getOptions = function (path, clonedParams, method) {
        var options = tslib.__assign({}, this._httpOptions);
...
function makeFetchCall(url, fetchOptions) {
    return fetch(url, tslib.__assign(tslib.__assign({}, fetchOptions), { credentials: 'same-origin' })).then(ResponseHandler.success, ResponseHandler.failure);
}
var ResponseHandler = {
...
var NodeApi = /** @class */ (function (_super) {
    tslib.__extends(NodeApi, _super);
    function NodeApi(options) {
        return _super.call(this, options) || this;
    }
    NodeApi.prototype.uploadFromStream = function (stream, filename) {
        var data = new NodeFormData__namespace();
...
//# sourceMappingURL=workfront-api.cjs.js.map
